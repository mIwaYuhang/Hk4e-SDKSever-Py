...
def mdk_guest_login():
    if not get_config()["auth"]["enable_server_guest"]:
        # todo: this seems to bug client out if already logged-in as guest
        return json_rsp_with_msg(define.RES_LOGIN_CANCEL, "Guest accounts are disabled.", {})
    ...
    try:
        ...
        if not guest:
            ...
            if not user:
                ...
                # account for which guest binding exists is not in db?
                return json_rsp_with_msg(define.RES_LOGIN_ERROR, "System error; please try again later.", {})
...
def mdk_shield_api_login():
    try:
        ...
        if not user:
            return json_rsp_with_msg(define.RES_LOGIN_FAILED, "你注册了吗？登录个der啊！！！", {})
...
def mdk_shield_api_verify():
    try:
        ...
        if token["device"] != request.headers.get('x-rpc-device_id'):
            # token was initially created on different device; forbid it
            return json_rsp_with_msg(define.RES_LOGIN_FAILED, "Game account cache information error.", {})
        ...
        if not user:
            ...
            # account for which token exists is not in db?
            return json_rsp_with_msg(define.RES_LOGIN_ERROR, "System error; please try again later.", {})
    ...
    try:
        ...
        return json_rsp_with_msg(define.RES_SUCCESS, "OK", {
            "data": {
                ...
                # CN only; forces game to send heartbeats so server can enforce maximum play time
                "heartbeat": False,
                "open_id": data["uid"],
...
def account_register():
    ...
    if request.method == 'POST':
        ...
        if user:
            ...
            flash(
                f"密码长度不能小于 {get_config()['security']['min_password_len']} 字符哦(づ￣ 3￣)づ", 'error')
        else:
            cursor.execute(
                "INSERT INTO `accounts` (`name`, `email`, `password`, `type`, `epoch_created`) VALUES (?, ?, ?, ?, ?)",
                (request.form.get('username'), request.form.get('email'), password_hash(
